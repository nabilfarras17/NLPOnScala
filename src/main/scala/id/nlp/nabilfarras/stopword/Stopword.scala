package id.nlp.nabilfarras.stopword

/**
  * Created by nabilfarras on 04/04/18.
  */
object Stopword {
  private val dictionary : List[String] = List(
  ...
  def removeStopWord(keyword : List[String]) : List[String] = {
    keyword.foldLeft[List[String]](List.empty){
      case (a,b) =>
        if(!dictionary.exists(item => item.contains(b.toLowerCase()))){
          a :+ b
        } else{
          a
        }
